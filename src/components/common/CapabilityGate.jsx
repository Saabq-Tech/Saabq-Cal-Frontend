...
export default function CapabilityGate({ capabilityCode, children }) {
  ...
  const description = !hasActiveSub
    ? (t('noSubscriptionDesc') || 'مساحة العمل الخاصة بك لا تمتلك اشتراكاً مفاعلاً. يرجى الاشتراك في إحدى الباقات للاستفادة من جميع الميزات.')
    : (t('capabilityLockedDesc') || 'الميزة المطلوبة تتطلب ترقية باقة الاشتراك الخاصة بمساحة العمل للوصول إليها واستخدامها.');
  ...
  return (
    <div
      ...
      style={{
        padding: 36,
        textAlign: 'center',
        background: 'linear-gradient(135deg, rgba(17, 100, 106, 0.04) 0%, rgba(17, 100, 106, 0.09) 100%)',
    ...
    >
      ...
      <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--heading)', marginBottom: 8 }}>
        {title}
      </h3>

      <p style={{ fontSize: '0.9rem', color: 'var(--muted)', maxWidth: 520, margin: '0 auto 24px', lineHeight: 1.6 }}>
        {description}
      </p>

      <div style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
        <Link to="/member/workspace/subscriptions" className="btn btn-primary btn-md" style={{ gap: 8 }}>
