    ...
    return (
      <div className="main-content">
        <div className="container profile-page animate-page-enter">
          ...
          <div style={{ padding: '80px 20px', textAlign: 'center', background: 'var(--surface)', borderRadius: 'var(--radius-lg, 16px)', boxShadow: 'var(--shadow-sm)', marginTop: 20 }}>
            <Icon name="shield" size={64} style={{ color: 'var(--border-strong)', marginBottom: 16 }} />
            <h2 style={{ color: 'var(--heading)', fontSize: '1.5rem', marginBottom: 8 }}>{t('noPermissionsTitle') || 'صلاحيات محدودة'}</h2>
            <p style={{ color: 'var(--text-secondary)', maxWidth: 400, margin: '0 auto 24px', lineHeight: 1.6 }}>
              {t('noPermissionsDesc') || 'ليس لديك أي صلاحيات لعرض أو إدارة إعدادات مساحة العمل. يرجى التواصل مع مالك مساحة العمل لمنحك الصلاحيات اللازمة.'}
            </p>
            <Link to="/member/profile" className="btn btn-primary" style={{ padding: '10px 24px' }}>
        ...
        <div className="profile-header">
          ...
          <div className="profile-info">
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
              {!isWorkspaceActive ? (
                <span className="profile-badge inactive" style={{ background: '#ef4444', color: '#fff', fontWeight: 800, border: '1px solid #dc2626' }}>
                  <span className="badge-dot inactive" style={{ background: '#fff' }} />
                  {t('workspaceInactiveBadge') || 'غير مفعّلة (بانتظار موافقة الإدارة)'}
                </span>
              ) : subStatus === 'pending' ? (
              ...
              <span className="profile-badge member" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                <Icon name="custom-b8c34186" size={12} />
                {t('workspace') || 'مساحة العمل'}
              </span>
              {isOwner && (
                <span className="profile-badge verified" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          ...
          <div style={{ marginInlineStart: 'auto' }}>
            <Link to="/member/profile" className="btn btn-primary btn-sm" style={{ gap: 6 }}>
              <Icon name="user" size={14} />
        ...
        <div className="profile-grid">
          <aside className="profile-sidebar">
            ...
            {availableTabs.map((wsTab) => {
              const isCapAllowed = isWorkspaceActive && checkWorkspaceCapability(user, wsTab.capability);
              ...
              if (!isWorkspaceActive) {
                return (
                  <div
                    ...
                    style={{ opacity: 0.5, cursor: 'not-allowed', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px' }}
                    title={t('workspaceInactiveTitle') || 'مساحة العمل غير مفعّلة'}
                  >
              ...
              return (
                <NavLink
                  key={wsTab.path}
                  to={wsTab.path}
                  className={({ isActive }) => `profile-sidebar-link${isActive ? ' active' : ''}`}
                ...
                >
                  ...
                  {!isCapAllowed && (
                    <Icon name="lock" size={14} style={{ color: 'var(--muted)', marginInlineStart: 6 }} />
                  )}
                </NavLink>
              );
            })}
          ...
          <main key={location.pathname} className="dashboard-content animate-fade-in-up" style={{ flex: 1, minWidth: 0 }}>
            {!isWorkspaceActive ? (
              <div style={{ padding: '60px 20px', textAlign: 'center', background: 'var(--surface)', borderRadius: 'var(--radius-lg, 16px)', border: '1px solid rgba(239, 68, 68, 0.2)', boxShadow: 'var(--shadow-sm)' }}>
                <Icon name="lock" size={56} style={{ color: '#ef4444', marginBottom: 16 }} />
                <h2 style={{ color: 'var(--heading)', fontSize: '1.4rem', fontWeight: 800, marginBottom: 8 }}>
                  {t('workspaceLockedTitle') || 'صفحات مساحة العمل مقفلة'}
                </h2>
                <p style={{ color: 'var(--text-secondary)', maxWidth: 460, margin: '0 auto 24px', lineHeight: 1.6, fontSize: '0.92rem' }}>
                  {t('workspaceLockedDesc') || 'لا يمكنك تصفح أو تعديل بيانات مساحة العمل لأن الحساب غير مفعّل بعد أو بانتظار موافقة أدمن المنصة. يمكنك الاستمرار في تعديل ملفك الشخصي واستعراض الدعم الفني.'}
                </p>
