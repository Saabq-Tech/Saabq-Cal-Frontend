...
export default function SocialLinksTab({ socialForm, setSocialForm: _setSocialForm, onSave, saving, canEdit }) {
  ...
  const [links, setLinks] = useState(() => {
    if (Array.isArray(socialForm.social_links) && socialForm.social_links.length > 0) {
    ...
    const initial = [];
    if (socialForm.website_url) initial.push({ platform: 'website', url: socialForm.website_url });
    if (socialForm.twitter_url) initial.push({ platform: 'x', url: socialForm.twitter_url });
    if (socialForm.linkedin_url) initial.push({ platform: 'linkedin', url: socialForm.linkedin_url });
    if (socialForm.instagram_url) initial.push({ platform: 'instagram', url: socialForm.instagram_url });
    if (socialForm.facebook_url) initial.push({ platform: 'facebook', url: socialForm.facebook_url });
  ...
  const DEFAULT_PLATFORMS = [
    {
      ...
      icon: (
        <Icon name="globe" size={16} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'x',
      label: 'X / Twitter',
      icon: (
        <Icon name="x-social" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'linkedin',
      label: 'LinkedIn',
      icon: (
        <Icon name="linkedin" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'instagram',
      label: 'Instagram',
      icon: (
        <Icon name="instagram" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'facebook',
      label: 'Facebook',
      icon: (
        <Icon name="facebook" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'youtube',
      label: 'YouTube',
      icon: (
        <Icon name="youtube" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'github',
      label: 'GitHub',
      icon: (
        <Icon name="github" size={15} style={{ display: 'block' }} />
      ),
    },
    {
      id: 'link',
      ...
      icon: (
        <Icon name="link" size={15} style={{ display: 'block' }} />
      ),
    },
  ];
  ...
  return (
    <form className="card-body" onSubmit={handleSubmit} style={{ gap: 20 }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border-light)', paddingBottom: 12 }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, margin: 0, color: 'var(--heading)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Icon name="link" size={20} style={{ color: 'var(--primary)' }} />
          {t('socialMediaLinks') || 'وسائل التواصل الاجتماعي والروابط'}
        </h3>
        <p style={{ fontSize: '0.84rem', color: 'var(--text-secondary)', margin: '4px 0 0' }}>
          {t('socialMediaLinksDesc') || 'روابط حساباتك وموقعك التي تظهر للعملاء في صفحة الحجز والإيميلات'}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {links.map((linkItem, idx) => {
          ...
          return (
            <div key={idx} style={{ background: 'var(--surface-alt)', padding: 16, borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12 }}>
                  ...
                  {platforms.map((plat) => {
                    ...
                    return (
                      <button
                        key={plat.id}
                        type="button"
                        onClick={() => canEdit && updateLinkPlatform(idx, plat.id)}
                        title={plat.label}
                        style={{
                          width: 36,
                          height: 36,
                ...
                {canEdit && links.length > 1 && (
                  ...
                  >
                    <Icon name="trash" size={16} style={{ display: 'block', margin: 'auto' }} />
                  </button>
                )}
              </div>
