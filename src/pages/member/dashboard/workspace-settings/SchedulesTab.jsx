...
export default function SchedulesTab({ schedules, startOfWeek = 'sunday', canEdit, onRefresh }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    client.get(endpoints.timezones)
      .then((res) => {
        if (res.data?.data) {
  ...
  const tzList = Array.isArray(timezones) && timezones.length > 0 ? timezones : [
    { id: 1, name: 'Asia/Riyadh', label: 'Asia/Riyadh (+03:00)', offset: '+03:00' },
    { id: 2, name: 'Asia/Dubai', label: 'Asia/Dubai (+04:00)', offset: '+04:00' },
    { id: 3, name: 'Africa/Cairo', label: 'Africa/Cairo (+03:00)', offset: '+03:00' },
    { id: 4, name: 'Europe/London', label: 'Europe/London (+00:00)', offset: '+00:00' },
    { id: 5, name: 'America/New_York', label: 'America/New_York (-05:00)', offset: '-05:00' },
  ];
  ...
  const normalizeSchedule = (sch) => {
    ...
    if (Array.isArray(sch.availability_rules) && sch.availability_rules.length > 0) {
      sch.availability_rules.forEach((rule) => {
        const day = parseInt(rule.day_of_week, 10);
        if (day >= 0 && day <= 6 && (rule.is_available ?? true)) {
          const from = rule.start_time ? String(rule.start_time).substring(0, 5) : '09:00';
          const to = rule.end_time ? String(rule.end_time).substring(0, 5) : '17:00';
    ...
    const exceptions = Array.isArray(sch.availability_overrides)
      ? sch.availability_overrides.map((ov) => ({
          ...
          reason: ov.reason || (ov.override_type === 'custom_hours' ? (t('customHours') || 'ساعات مخصصة') : (t('vacationClosedDay') || 'إجازة / يوم مقفول')),
          from_time: ov.start_time ? String(ov.start_time).substring(0, 5) : null,
  ...
  const [selectedScheduleId, setSelectedScheduleId] = useState(() => {
    const active = (Array.isArray(schedules) ? schedules : []).find(s => s.is_default) || schedules?.[0];
    return active?.id || null;
  });

  useEffect(() => {
    if (schedulesList.length > 0 && !schedulesList.some(s => s.id === selectedScheduleId)) {
  ...
  const handleSaveModalForm = async (e) => {
    ...
    try {
      ...
      if (modalForm.editing_id) {
        await client.put(endpoints.workspaceScheduleItem(modalForm.editing_id), payload);
        toast.success(t('scheduleUpdatedSuccess') || 'تم تحديث البيانات بنجاح');
      } else {
        await client.post(endpoints.workspaceSchedules, payload);
        toast.success(t('scheduleCreatedSuccess') || 'تم إنشاء الجدول الجديد بنجاح');
      }
      ...
      toast.error(err.response?.data?.message || t('saveScheduleFailed') || 'فشل حفظ الجدول');
    } finally {
  ...
  const handleDeleteSchedule = async () => {
    ...
    if (activeSchedule.is_default) {
      toast.error(t('cannotDeleteDefaultSchedule') || 'لا يمكن حذف الجدول الافتراضي لمساحة العمل');
      return;
    }

    const confirmMsg = (t('confirmDeleteSchedule') || 'هل أنت تأكد من رغبتك في حذف هذا الجدول؟');
    ...
    setSchedulesList((prev) =>
      prev.map((s) => {
        if (s.id === selectedScheduleId) {
          ...
          const updatedSlots = currentSlots.length > 0 ? [] : [{ from: '09:00', to: '17:00' }];
          return {
            ...s,
            weekly_hours: {
              ...(s.weekly_hours || {}),
      ...
      prev.map((s) => {
        if (s.id === selectedScheduleId) {
          ...
          if (currentSlots[slotIdx]) {
            currentSlots[slotIdx] = { ...currentSlots[slotIdx], [field]: value };
          }
          return {
            ...s,
            weekly_hours: {
  ...
  const handleSaveWeeklyRules = async () => {
    ...
    try {
      ...
      const rules = Object.keys(activeSchedule.weekly_hours || {}).map((dayKey) => {
      ...
      await client.put(`${endpoints.workspaceSchedules}/${activeSchedule.id}/weekly-rules`, { rules });
      toast.success(t('weeklyRulesSavedSuccess') || 'تم حفظ ساعات العمل الأسبوعية بنجاح');
      if (onRefresh) {
        onRefresh();
      }
    } catch (err) {
      console.error('Failed to save weekly rules:', err);
      toast.error(err.response?.data?.message || 'فشل حفظ ساعات العمل الأسبوعية');
    } finally {
  ...
  const handleAddException = async (e) => {
    ...
    try {
      ...
      await client.post(`${endpoints.workspaceSchedules}/${activeSchedule.id}/overrides`, payload);
      setExceptionForm({ date: '', type: 'closed', reason: '', from_time: '09:00', to_time: '13:00' });
  ...
  const handleDeleteException = async (exId) => {
    if (!canEdit || !activeSchedule) return;
    try {
      await client.delete(`${endpoints.workspaceSchedules}/${activeSchedule.id}/overrides/${exId}`);
  ...
  return (
    <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, paddingBottom: 16, borderBottom: '1px solid var(--border-light)' }}>
        <div>
          ...
          <p style={{ fontSize: '0.86rem', color: 'var(--text-secondary)', margin: 0 }}>
            {t('workspaceSchedulesDesc') || 'ضبط أوقات وأيام العمل الفعالة لكل أسبوع، فترات الصلاحية، والاستثناءات المخصصة'}
          </p>
        </div>

        {canEdit && (
          <button className="btn btn-primary btn-sm" onClick={handleOpenCreateModal} style={{ gap: 6 }}>
      ...
      {schedulesList.length === 0 ? (
        <div style={{ padding: '48px 20px', textAlign: 'center', background: 'var(--surface-alt)', borderRadius: 'var(--radius-lg)', border: '1px border-dashed var(--border)' }}>
          ...
          <h4 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 6px', color: 'var(--heading)' }}>
            {t('noSchedulesFound') || 'لا توجد جداول عمل أو ساعات توفر مضافة حالياً'}
          </h4>
          <p style={{ fontSize: '0.85rem', color: 'var(--muted)', margin: '0 0 16px' }}>
            {t('noSchedulesDesc') || 'قم بإنشاء جدول العمل الأول لمساحتك لتحديد أيام وساعات العمل المتاحة.'}
          </p>
          {canEdit && (
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, background: 'var(--surface-alt)', padding: 12, borderRadius: 'var(--radius-lg)' }}>
            ...
            {schedulesList.map((sch) => {
              ...
              return (
                ...
                >
                  <span>{sch.name}</span>
                  {sch.is_default && (
                    <span style={{ fontSize: '0.7rem', background: 'var(--primary)', color: '#ffffff', padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>
        ...
        {activeSchedule && canEdit && (
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              ...
              style={{ fontSize: '0.8rem', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 4 }}
            >
            ...
            <button
              ...
              title={activeSchedule.is_default ? (t('cannotDeleteDefaultSchedule') || 'لا يمكن حذف الجدول الافتراضي لمساحة العمل') : (t('deleteScheduleBtn') || 'حذف الجدول')}
              style={{
                fontSize: '0.8rem',
                fontWeight: 600,
                color: activeSchedule.is_default ? 'var(--muted)' : '#ef4444',
                opacity: activeSchedule.is_default ? 0.5 : 1,
                cursor: activeSchedule.is_default ? 'not-allowed' : 'pointer',
                display: 'inline-flex',
                alignItems: 'center',
                gap: 4,
              }}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: isDayEnabled ? '1px solid var(--border-light)' : '1px solid var(--border)',
                    display: 'flex',
                ...
                >
                  {/* Day Checkbox & Title */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 140, paddingTop: 6 }}>
                    <input
                      type="checkbox"
                      checked={isDayEnabled}
                      onChange={() => handleToggleDay(d.key)}
                      disabled={!canEdit}
                      style={{ accentColor: 'var(--primary)', width: 18, height: 18, cursor: canEdit ? 'pointer' : 'default' }}
                    />
                    <span style={{ fontWeight: 800, fontSize: '0.96rem', color: isDayEnabled ? 'var(--heading)' : 'var(--muted)' }}>
                      {d.label}
                    </span>
                  </div>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {isDayEnabled ? (
                      slots.map((slot, sIdx) => (
                        <div key={sIdx} style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <input
                            type="time"
                            className="form-input"
                            value={slot.from}
                            onChange={(e) => handleSlotChange(d.key, sIdx, 'from', e.target.value)}
                            disabled={!canEdit}
                            style={{ width: 120, height: 36, fontSize: '0.84rem', padding: '0 8px' }}
                          />
                          <span style={{ fontSize: '0.82rem', color: 'var(--muted)' }}>إلى</span>
                          <input
                            type="time"
                            className="form-input"
                            value={slot.to}
                            onChange={(e) => handleSlotChange(d.key, sIdx, 'to', e.target.value)}
                            disabled={!canEdit}
                            style={{ width: 120, height: 36, fontSize: '0.84rem', padding: '0 8px' }}
                          />
                      ...
                      <span style={{ fontSize: '0.84rem', color: 'var(--muted)', paddingTop: 6 }}>{t('unavailableClosed') || 'غير متاح (مغلق)'}</span>
                    )}
                  </div>
      ...
      <div className="card" style={{ padding: 24, border: '1px solid var(--border-light)', borderRadius: 'var(--radius-lg)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          <Icon name="calendar" size={20} />
          <h3 style={{ fontSize: '1.08rem', fontWeight: 800, margin: 0, color: 'var(--heading)' }}>
        ...
        <p style={{ fontSize: '0.84rem', color: 'var(--text-secondary)', marginBottom: 20, lineHeight: 1.5 }}>
          {t('scheduleValiditySub') || 'حدد تاريخ بداية و/أو نهاية لسريان هذا الجدول المتكرر. بعد تاريخ النهاية يتوقف عرض المواعيد تلقائياً، اتركها فارغة لجدول دائم.'}
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
          <div className="form-group">
            <label className="form-label" style={{ fontWeight: 700 }}>{t('validFromLabel') || 'ساري من'}</label>
            <input
              type="date"
              className="form-input"
              value={validityForm.valid_from}
              onChange={(e) => setValidityForm({ ...validityForm, valid_from: e.target.value })}
              disabled={!canEdit}
            />
          </div>

          <div className="form-group">
            <label className="form-label" style={{ fontWeight: 700 }}>{t('validUntilLabel') || 'ساري حتى'}</label>
            <input
              type="date"
              className="form-input"
              value={validityForm.valid_until}
              onChange={(e) => setValidityForm({ ...validityForm, valid_until: e.target.value })}
              disabled={!canEdit}
            />
          </div>
        </div>
      ...
      <div className="card" style={{ padding: 24, border: '1px solid var(--border-light)', borderRadius: 'var(--radius-lg)' }}>
        ...
        <p style={{ fontSize: '0.84rem', color: 'var(--text-secondary)', marginBottom: 20, lineHeight: 1.5 }}>
          {t('exceptionsSub') || 'حدد يوماً مقفولاً تماماً، أو يوماً بساعات مختلفة عن المعتاد'}
        </p>

        {/* Inline Add Exception Form */}
        {canEdit && (
          <form onSubmit={handleAddException} style={{ background: 'var(--surface-alt)', padding: 18, borderRadius: 'var(--radius-md)', border: '1px solid var(--border-light)', marginBottom: 20 }}>
            <div className="form-group" style={{ marginBottom: 14 }}>
              <label className="form-label" style={{ fontWeight: 700 }}>{t('selectDateLabel') || 'اختر التاريخ'}</label>
              <input
                type="date"
                className="form-input"
            ...
            <div style={{ display: 'flex', gap: 20, marginBottom: 14 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontWeight: 700, fontSize: '0.86rem' }}>
                <input
                  type="radio"
                  name="ex_type"
                  checked={exceptionForm.type === 'closed'}
                  onChange={() => setExceptionForm({ ...exceptionForm, type: 'closed' })}
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontWeight: 700, fontSize: '0.86rem' }}>
                <input
                  ...
                  onChange={() => setExceptionForm({ ...exceptionForm, type: 'custom' })}
                  style={{ accentColor: 'var(--primary)' }}
                />
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setExceptionForm({ ...exceptionForm, from_time: e.target.value })}
                  />
                </div>
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setExceptionForm({ ...exceptionForm, to_time: e.target.value })}
                  />
                </div>
              </div>
            )}
            ...
            <button type="submit" className="btn btn-primary btn-sm" style={{ width: '100%', justifyContent: 'center', fontWeight: 700 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {(!activeSchedule.exceptions || activeSchedule.exceptions.length === 0) ? (
            <div style={{ textAlign: 'center', padding: 24, color: 'var(--muted)', fontSize: '0.86rem' }}>
              {t('noExceptionsFound') || 'لا توجد استثناءات'}
            </div>
          ) : (
            activeSchedule.exceptions.map((ex) => (
              <div key={ex.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'var(--surface-alt)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-light)' }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--heading)', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span>📅 {ex.date}</span>
                    <span className={`profile-badge ${ex.type === 'closed' ? 'unverified' : 'verified'}`} style={{ fontSize: '0.74rem', padding: '2px 8px' }}>
                      {ex.type === 'closed' ? (t('typeClosedDay') || 'يوم مقفول') : (t('typeCustomHours') || 'ساعات مخصصة')}
                    </span>
                  </div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: 4 }}>
                    {ex.reason} {ex.type === 'custom' && `(${ex.from_time} - ${ex.to_time})`}
                  </div>
                </div>
          ...
          <div className="modal-card modal-sm animate-scale-up">
            <div className="modal-header">
              <h3 className="modal-title">
                {modalForm.editing_id ? (t('editScheduleModalTitle') || 'تعديل بيانات الجدول') : (t('newScheduleModalTitle') || 'جدول جديد')}
              </h3>
              <button type="button" className="modal-close-btn" onClick={() => setIsScheduleModalOpen(false)}>
                <Icon name="x" size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveModalForm} className="modal-body">
              <div className="form-group" style={{ marginBottom: 14 }}>
                <label className="form-label" style={{ fontWeight: 700 }}>{t('scheduleNameLabel') || 'الاسم *'}</label>
                <input
                  type="text"
                  className="form-input"
                  value={modalForm.name}
                  onChange={(e) => setModalForm({ ...modalForm, name: e.target.value })}
                  placeholder={t('scheduleNamePlaceholder') || 'مثال: الجدول الافتراضي'}
                  required
                />
              </div>

              <div className="form-group" style={{ marginBottom: 14 }}>
                <label className="form-label" style={{ fontWeight: 700 }}>{t('memberScopeLabel') || 'جدول العضو (السبب (اختياري))'}</label>
                <select
                  className="form-select"
                  value={modalForm.scope}
                  onChange={(e) => setModalForm({ ...modalForm, scope: e.target.value })}
                >
                  <option value="workspace">{t('workspaceLevelScope') || 'مستوى مساحة العمل'}</option>
                  <option value="member">{t('specificMemberScope') || 'عضو محدد في مساحة العمل'}</option>
                </select>
              </div>

              <div className="form-group" style={{ marginBottom: 14 }}>
                <label className="form-label" style={{ fontWeight: 700 }}>{t('timezoneLabel') || 'المنطقة الزمنية'}</label>
                <SearchableSelect
              ...
              <div className="form-group" style={{ marginBottom: 20 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontWeight: 700, fontSize: '0.88rem' }}>
                  <input
                    ...
                    onChange={(e) => setModalForm({ ...modalForm, is_default: e.target.checked })}
                    disabled={activeSchedule?.is_default && modalForm.editing_id === activeSchedule.id}
                    style={{ accentColor: 'var(--primary)', width: 18, height: 18 }}
                  />
                  <span>{t('isDefaultScheduleLabel') || 'الجدول الافتراضي'}</span>
                </label>
              </div>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn btn-primary btn-sm" disabled={savingModal} style={{ fontWeight: 700 }}>
                  {savingModal ? (
                    <>
                      <span className="spinner spinner-sm" style={{ borderTopColor: '#fff' }} />
                      {t('saving') || 'جاري الحفظ...'}
