...
export default function IntegrationsSettingsPage() {
  ...
  const [telegramEnableActionButtons, setTelegramEnableActionButtons] =
    useState(true);
  const [telegramActionButtonsConfig, setTelegramActionButtonsConfig] =
    useState("");
  ...
  const loadSecurityData = async () => {
    ...
    if (tRes?.success && tRes.data) {
      ...
      setTelegramChats(
        tRes.data.chats && tRes.data.chats.length > 0
          ? tRes.data.chats
          : [
              {
                id: 1,
      ...
      setTelegramActionButtonsConfig(
        tRes.data.action_buttons_config
          ? JSON.stringify(tRes.data.action_buttons_config, null, 2)
          : JSON.stringify(defaultTelegramActionConfig, null, 2),
      );
  ...
  const handleStartGoogleOAuth = () => {
          ...
          openConfirm(
            ...
            async () => {
              ...
              if (res.data.expires_in)
                retryPayload.expires_in = res.data.expires_in;
              ...
              if (retryRes.success) {
                ...
                toast.error(
                  retryRes.message || "Failed to connect Google account.",
                );
              }
            },
          );
    ...
    if (window.google?.accounts?.oauth2?.initCodeClient) {
      const codeClient = window.google.accounts.oauth2.initCodeClient({
        client_id: clientId,
        scope:
          "https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/spreadsheets https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile openid",
        ux_mode: "popup",
  ...
  const handleSaveCalendarSettings = async () => {
    ...
    if (res.success) {
      toast.success(
        t("calendarSettingsSaved") || "تم حفظ إعدادات التقويم بنجاح",
      );
  ...
  const handleSaveWebhookSettings = async () => {
    ...
    if (webhookEvents.booking_confirmed)
      selectedEvents.push("booking.confirmed");
    if (webhookEvents.booking_cancelled)
      selectedEvents.push("booking.cancelled");
    ...
    if (res?.success) {
      toast.success(
        res.message ||
          t("webhookSettingsSaved") ||
          "تم حفظ رابط الـ Webhook والأحداث بنجاح",
      );
      await loadSecurityData();
      setActiveModalId(null);
    } else {
    ...
    openConfirm(
      ...
      async () => {
        ...
        if (res?.success) {
          toast.success(
            res.message ||
              t("webhookDisconnectedSuccess") ||
              "تم إلغاء رابط الـ Webhook بنجاح",
          );
          setWebhookUrl("");
          await loadSecurityData();
          setActiveModalId(null);
  ...
  const handleSaveTelegramSettings = async () => {
    ...
    if (telegramActionButtonsConfig.trim()) {
      try {
        ...
        toast.error(
          t("invalidJsonConfig") || "تنسيق JSON غير صالح لإعدادات الأزرار",
        );
        return;
      }
    }
    ...
    if (res?.success) {
      toast.success(
        res.message ||
          t("telegramSettingsSaved") ||
          "تم حفظ إعدادات Telegram بنجاح",
      );
      await loadSecurityData();
      setActiveModalId(null);
    } else {
    ...
    openConfirm(
      ...
      async () => {
        ...
        if (res?.success) {
          toast.success(
            res.message ||
              t("telegramDisconnectedSuccess") ||
              "تم إلغاء ربط Telegram بنجاح",
          );
          await loadSecurityData();
          setActiveModalId(null);
        } else {
  ...
  const handleSaveEmailSettings = async () => {
    ...
    if (
      mailDriver === "resend" &&
      !resendApiKey &&
      !emailIntegration?.has_resend_api_key
    ) {
    ...
    if (res?.success) {
      ...
      const firstError = res?.errors
        ...
        : null;
      toast.error(
        firstError || res?.message || "Failed to save Email settings.",
      );
    }
  };
    ...
    openConfirm(
      ...
      async () => {
        ...
        if (res?.success) {
          toast.success(
            res.message ||
              t("emailDisconnectedSuccess") ||
              "تم إلغاء ربط البريد الإلكتروني بنجاح",
          );
          await loadSecurityData();
          setActiveModalId(null);
        } else {
          toast.error(
            res?.message || "Failed to disconnect Email integration.",
          );
        }
      },
    );
  ...
  const integrationsList = [
    {
      ...
      subtitle: googleIntegration
        ? `${t("connectedAs") || "مرتبط كـ:"} ${googleIntegration.google_email}`
        : t("statusNotConnected") || "غير مرتبط",
    },
    {
      id: "google_calendar",
      title: "Google Calendar",
      category: "google",
      icon: <Icon name="google-calendar" size={28} />,
      description:
      ...
      subtitle: calendarSync
        ? t("syncEnabled") || "المزامنة مفعّلة"
        : t("statusDisabled") || "معطّلة",
    },
    {
      id: "google_meet",
      title: "Google Meet",
    ...
    },
    {
      ...
      category: "automation",
      icon: <Icon name="telegram" size={28} />,
      description:
        t("telegramDesc") ||
    ...
    },
    {
      ...
      description:
        t("notificationsDesc") ||
        "إعدادات البريد الإلكتروني (SMTP / Resend) لإرسال إشعارات الحجوزات والتنبيهات المباشرة فور حجز أي موعد جديد.",
  ...
  return (
    ...
    >
          ...
          >
            <div>
              ...
              >
                ...
                <span
                  className={`profile-badge ${item.isConnected ? "verified" : "unverified"}`}
                  style={{
                    fontSize: "0.75rem",
                    padding: "3px 10px",
                    whiteSpace: "nowrap",
            ...
            <div className="modal-card animate-scale-up">
              ...
              {googleIntegration ? (
                ...
                >
                  ...
                  >
                    {t("connectedAccountLabel") || "الحساب المرتبط حالياً"}
                  </div>
                  <div
                    style={{
                      fontSize: "1.05rem",
                  ...
                  >
                    {t("connectionDate") || "تاريخ الربط:"}{" "}
                    {new Date(
                      googleIntegration.created_at || Date.now(),
                    ).toLocaleDateString(
      ...
      {activeModalId === "google_calendar" &&
        createPortal(
          <div className="modal-backdrop">
            <div className="modal-card animate-scale-up">
              <div className="modal-header">
                <h3 className="modal-title">
                  {t("googleCalendarSettingsTitle")}
                </h3>
                <button
                  type="button"
                  className="modal-close-btn"
          ...
          <div className="modal-backdrop">
            <div className="modal-card animate-scale-up">
              ...
              >
                <div>
                  <div style={{ fontWeight: 700, fontSize: "0.92rem" }}>
                    {t("autoCreateMeetLinksLabel") ||
                      "إنشاء رابط اجتماع Google Meet تلقائياً"}
                  </div>
                  <div
                    style={{
                      fontSize: "0.8rem",
      ...
      {activeModalId === "google_sheets" &&
        createPortal(
          <div className="modal-backdrop">
            <div className="modal-card animate-scale-up">
              <div className="modal-header">
                <h3 className="modal-title">
                  {t("googleSheetsSettingsTitle")}
                </h3>
                <button
                  type="button"
                  className="modal-close-btn"
              ...
              >
                <div>
                  <div style={{ fontWeight: 700, fontSize: "0.92rem" }}>
                    {t("sheetsExportSyncLabel") ||
                      "تفعيل التصدير والمزامنة التلقائية"}
                  </div>
                  <div style={{ fontSize: "0.8rem", color: "var(--muted)" }}>
                    {t("sheetsExportSyncSub") ||
                      "إضافة كل حجز جديد كصف بالجدول"}
                  </div>
                </div>
                <ToggleSwitch
                  checked={sheetsSync}
              ...
              <div className="form-group" style={{ marginBottom: 14 }}>
                <label className="form-label" style={{ fontWeight: 700 }}>
                  {t("sheetHeaderLangLabel") || "لغة العناوين بالجدول"}
                </label>
                <select
              ...
              >
                  ...
                  >
                      ...
                      <div>
                        ...
                        >
                          {t("telegramCustomBotDesc") ||
                            "أنشئ بوتًا خاصًا بك عبر BotFather وأدخل الـ Token الخاص به."}
                        </div>
                      </div>
                    </label>
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {t("telegramDefaultBot") ||
                            "استخدام بوت Saabq Cal الافتراضي"}
                        </div>
                        <div
                          style={{
                            fontSize: "0.8rem",
                ...
                {telegramBotType === "default" && (
                  ...
                  >
                    ...
                    >
                      ...
                      {t("openBotAndConnect") ||
                        "فتح البوت وربط محادثة تلقائيًا"}
                    </a>
                    <div
                      style={{
                        fontSize: "0.78rem",
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      placeholder={
                        ...
                        "أدخل الـ Token الخاص بالبوت هنا"
                      }
                ...
                {telegramChats.map((chat, idx) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {telegramChats.length > 1 && (
                        <button
                          ...
                          onClick={() => {
                            const updated = telegramChats.filter(
                              (_, i) => i !== idx,
                            );
                    ...
                    <div style={{ marginTop: 14 }}>
                      ...
                      >
                        {t("servicesToNotify") ||
                          "الخدمات التي تصل إشعاراتها إلى هذا الشات"}
                      </div>
                      <div
                        style={{
                          fontSize: "0.78rem",
                      ...
                      >
                        {t("servicesToNotifyNotice") ||
                          "أدخل واحفظ الـ Chat ID أولاً قبل تحديد الخدمات."}
                      </div>
                      ...
                      {services && services.length > 0 ? (
                        ...
                        >
                          {services.map((srv) => {
                            const isChecked = (chat.service_ids || []).includes(
                              srv.id,
                            );
                            return (
                              <label
                ...
                >
                  ...
                  <ToggleSwitch
                    ...
                    onChange={(e) =>
                      setTelegramEnableActionButtons(e.target.checked)
                    }
                  />
                </div>
                ...
                {telegramEnableActionButtons && (
                  <div className="form-group" style={{ marginTop: 16 }}>
                    ...
                    >
                      {t("actionButtonsConfig") ||
                        "إعدادات الأزرار المخصصة (JSON)"}
                      <span
                        style={{
                          fontSize: "0.75rem",
                          fontWeight: 400,
      ...
      {activeModalId === "webhooks" &&
        createPortal(
          <div className="modal-backdrop">
            <div className="modal-card animate-scale-up">
              <div className="modal-header">
                <h3 className="modal-title">
                  {t("webhooksModalTitle") ||
                    "الربط البرمجي (Webhooks) والتنبيهات"}
                </h3>
                <button
                  type="button"
                  className="modal-close-btn"
              ...
              <div className="form-group" style={{ marginBottom: 16 }}>
                <label className="form-label" style={{ fontWeight: 700 }}>
                  {t("webhookUrlLabel") || "رابط الـ Webhook *"}
                </label>
                <input
                  type="url"
                  className="form-input"
                  placeholder={
                    t("webhookUrlPlaceholder") ||
                    "https://api.yourdomain.com/webhooks/saabq"
                  }
                  value={webhookUrl}
              ...
              <label
                className="form-label"
                style={{ fontWeight: 700, marginBottom: 8, display: "block" }}
              >
                {t("triggerEventsLabel") || "الأحداث المطلوبة للتنبيه:"}
              </label>
              <div
                style={{
                  display: "flex",
              ...
              >
                {[
                  {
                    id: "booking_created",
                    label: `${t("bookingCreatedEvent") || "إنشاء حجز جديد"} (booking.created)`,
                  },
                  {
                    id: "booking_confirmed",
                    label: `${t("bookingConfirmedEvent") || "تأكيد موعد الحجز"} (booking.confirmed)`,
                  },
                  {
                    id: "booking_cancelled",
                    label: `${t("bookingCancelledEvent") || "إلغاء حجز الموعد"} (booking.cancelled)`,
                  },
                ].map((evt) => (
                  <label
                    key={evt.id}
                    ...
                    <div className="form-group">
                      ...
                      <div style={{ position: "relative" }}>
                        <input
                          ...
                          style={{
                            paddingLeft: t("lang") === "en" ? 70 : 12,
                            paddingRight: t("lang") === "en" ? 12 : 70,
                          }}
                        />
                        <button
                          type="button"
                      ...
                      <div className="form-group">
                        ...
                        <div style={{ position: "relative" }}>
                          <input
                            ...
                            className="form-input"
                            placeholder={
                              emailIntegration?.has_smtp_password
                                ? "••••••••"
                                : "••••••••"
                            }
                            value={smtpPassword}
                            ...
                            style={{
                              paddingLeft: t("lang") === "en" ? 65 : 12,
                              paddingRight: t("lang") === "en" ? 12 : 65,
                            }}
                          />
                          <button
                            type="button"
