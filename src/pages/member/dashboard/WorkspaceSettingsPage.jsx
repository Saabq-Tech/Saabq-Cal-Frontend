...
export default function WorkspaceSettingsPage() {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      ...
      setLoading(true);
      try {
        const [settingsRes, tmplRes, tzRes, typesRes, countriesRes] = await Promise.all([
          client.get(endpoints.workspaceSettings),
          client.get(endpoints.workspaceSettingsNotifications).catch(() => null),
        ...
        if (data) {
          setSettings(data);
          applyWorkspaceBranding(data.primary_color, data.secondary_color, data.hover_color);
          setBasicForm({
            name: data.name || '',
            email: data.email || '',
            phone: data.phone || '',
          ...
          const tzStr = typeof data.timezone === 'object' ? (data.timezone?.name || 'Asia/Riyadh') : (data.timezone || 'Asia/Riyadh');
          ...
          setBookingRulesForm({
            ...
            default_buffer_before_minutes: data.default_buffer_before_minutes ?? 0,
            default_buffer_after_minutes: data.default_buffer_after_minutes ?? 0,
            minimum_booking_notice_minutes: data.minimum_booking_notice_minutes ?? 0,
          ...
          setPaymentReceiptsForm({
            payment_receipt_mode: data.payment_receipt_mode || data.receipt_mode || 'required',
            receipt_mode: data.payment_receipt_mode || data.receipt_mode || 'required',
          });
        ...
        if (tmplRes?.data?.data && Array.isArray(tmplRes.data.data) && tmplRes.data.data.length > 0) {
  ...
  const handleSaveSection = async (data, targetEndpoint = endpoints.workspaceSettings) => {
    if (!canEdit) {
      toast.error(t('unauthorizedSettingsEdit') || 'لا تملك صلاحية تعديل إعدادات مساحة العمل');
      return;
    }
    setSaving(true);
    try {
      ...
      toast.success(res.data?.message || t('settingsUpdatedSuccess') || 'تم حفظ الإعدادات بنجاح');
      if (fetchProfile) {
        await fetchProfile();
      }
  ...
  const handleSaveNotificationTemplate = async (key, formData) => {
    if (!canEdit || settings?.allow_template_editing === false) {
      toast.error(t('templateEditingDisabledNotice') || 'تعديل قوالب الإشعارات معطل لمساحة العمل من قبل الأدمن');
      return;
    }
    setSaving(true);
    try {
      const res = await client.put(`${endpoints.workspaceSettingsNotifications}/${key}`, formData);
      ...
      toast.success(res.data?.message || t('settingsUpdatedSuccess') || 'تم تحديث قالب الإشعار بنجاح');
    } catch (err) {
  ...
  const settingsSubTabs = [
    {
      ...
      icon: (
        <Icon name="custom-34f286e2" size={15} />
      ),
    },
    {
      id: 'branding',
      ...
      icon: (
        <Icon name="custom-ecd73178" size={15} />
      ),
    },
    {
      id: 'timezone',
    ...
    },
    {
      ...
      icon: (
        <Icon name="custom-3f7c7395" size={15} />
      ),
    },
    {
      id: 'form_fields',
  ...
  return (
    <div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  whiteSpace: 'nowrap',
                  transition: 'all 0.2s ease',
                  boxShadow: isSubActive ? '0 2px 8px rgba(17,100,106,0.22)' : 'none',
                  flexShrink: 0,
                }}
              >
                <span style={{ display: 'inline-flex', alignItems: 'center', color: 'currentColor' }}>{sub.icon}</span>
                <span>{sub.label}</span>
              </button>
            );
          })}
      ...
      <div key={subSettingsTab} className="card animate-tab-card" style={{ padding: 24 }}>
        {loading ? (
          ...
          <>
            {subSettingsTab === 'basic' && <BasicInfoTab basicForm={basicForm} setBasicForm={setBasicForm} workspaceTypes={workspaceTypes} countries={countries} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsBasic)} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'branding' && <BrandingTab brandingForm={brandingForm} setBrandingForm={setBrandingForm} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsBranding)} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'timezone' && <TimezoneTab timezoneForm={timezoneForm} setTimezoneForm={setTimezoneForm} timezones={timezones} onSave={handleSaveSection} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'social' && <SocialLinksTab socialForm={socialForm} setSocialForm={setSocialForm} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsSocial)} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'booking_rules' && <BookingRulesTab bookingRulesForm={bookingRulesForm} setBookingRulesForm={setBookingRulesForm} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsBookingRules)} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'form_fields' && (
              <BookingFormFieldsTab formFieldsForm={formFieldsForm} setFormFieldsForm={setFormFieldsForm} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsBookingForm)} saving={saving} canEdit={canEdit} />
            )}
            {subSettingsTab === 'payment' && <PaymentReceiptsTab paymentReceiptsForm={paymentReceiptsForm} setPaymentReceiptsForm={setPaymentReceiptsForm} onSave={(data) => handleSaveSection(data, endpoints.workspaceSettingsPayment)} saving={saving} canEdit={canEdit} />}
            {subSettingsTab === 'notifications' && (
              <CapabilityGate capabilityCode="CUSTOM_TEMPLATES">
                <NotificationTemplatesTab
                  ...
                  saving={saving}
                  canEdit={canEdit && settings?.allow_template_editing !== false}
                  allowTemplateEditing={settings?.allow_template_editing !== false}
