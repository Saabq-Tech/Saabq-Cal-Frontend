...
export default function WorkspaceServicesPage() {
  ...
  const canRead = isOwner || userPermissions.includes('service_read') || userPermissions.includes('services_read');
  const canEdit = isOwner || userPermissions.includes('service_write') || userPermissions.includes('services_write');
  ...
  const loadServices = async () => {
    ...
    try {
      ...
      const [srvRes, memRes] = await Promise.all([
        ...
        client.get(endpoints.workspaceMembers).catch(() => ({ data: { data: [] } })),
      ]);
  ...
  const handleSaveService = async (serviceForm) => {
    try {
      ...
      if (serviceId) {
        await client.put(endpoints.workspaceServiceItem(serviceId), serviceForm);
    ...
    <CapabilityGate capabilityCode="BOOKING">
      <div className="card" style={{ padding: 24 }}>
        ...
        {loading ? (
          ...
          <ServicesTab services={services} members={members} canEdit={canEdit} onSaveService={handleSaveService} />
        )}
      </div>
    </CapabilityGate>
  );
